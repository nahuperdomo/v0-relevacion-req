...
import { Search, Eye, Download, TrendingUp, AlertCircle, CheckCircle2, Clock, BarChart3, FileText, Users, MessageSquare, Target } from "lucide-react"
...
export default function ReportesPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const [resultsData, sectionsData, interviewsData] = await Promise.all([
        ...
        sectionsApi.getAll(),
        interviewsApi.getAll({ status: 'COMPLETED', limit: 50 })
      ])
  ...
  const completedInterviewsWithSummary = interviews.filter(
    (int) => int.status === 'COMPLETED' && (int as any).general_summary
  )
          ...
          <TabsContent value="consolidados" className="space-y-6 mt-6">
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              <Card className="border-border/50">
                ...
                <CardContent>
                  <div className="text-3xl font-bold">{completedInterviewsWithSummary.length}</div>
              ...
              <Card className="border-border/50">
                ...
                <CardContent>
                  <div className="text-3xl font-bold text-red-400">
                    {completedInterviewsWithSummary.reduce((sum, int) => {
                      const issues = (int as any).critical_issues
              ...
              <Card className="border-border/50">
                ...
                <CardContent>
                  <div className="text-3xl font-bold text-amber-400">
                    {completedInterviewsWithSummary.length > 0
                      ? (completedInterviewsWithSummary.reduce((sum, int) => sum + ((int as any).average_urgency || 0), 0) / completedInterviewsWithSummary.length).toFixed(1)
                      : '0.0'}
                  </div>
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                {completedInterviewsWithSummary.map((interview) => {
                  const interviewAny = interview as any
                  const resultCount = results.filter(r => r.interviewId === interview.interview_id).length
                  
                  return (
                    <Card key={interview.interview_id} className="border-border/50">
                      <CardHeader>
                          ...
                          <div className="space-y-1">
                            ...
                            <CardDescription className="flex items-center gap-4 text-sm">
                              ...
                              <span className="flex items-center gap-1.5">
                                ...
                                {resultCount} empleados entrevistados
                              </span>
                              {interviewAny.overall_sentiment && (
                                <Badge 
                                  variant="outline" 
                                  className={`gap-1.5 ${getSentimentColor(interviewAny.overall_sentiment)}`}
                                >
                                  {interviewAny.overall_sentiment === 'positive' && <CheckCircle2 className="h-3.5 w-3.5" />}
                                  {interviewAny.overall_sentiment === 'negative' && <AlertCircle className="h-3.5 w-3.5" />}
                                  {interviewAny.overall_sentiment === 'neutral' && <Clock className="h-3.5 w-3.5" />}
                                  {interviewAny.overall_sentiment === 'mixed' && <MessageSquare className="h-3.5 w-3.5" />}
                                  {interviewAny.overall_sentiment}
                                </Badge>
                              )}
                            </CardDescription>
                      ...
                      <CardContent className="space-y-4">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground leading-relaxed">
                            {interviewAny.general_summary || 'No disponible'}
                          </p>
                        </div>

                        <div className="grid md:grid-cols-2 gap-4">
                          {interviewAny.consolidated_topics && interviewAny.consolidated_topics.length > 0 && (
                            <div>
                              <h4 className="text-sm font-semibold mb-2">🎯 Temas Principales</h4>
                              <div className="flex flex-wrap gap-2">
                                {interviewAny.consolidated_topics.slice(0, 5).map((topic: string) => (
                                  <Badge key={topic} variant="outline" className="bg-violet-500/5">
                                    {topic}
                                  </Badge>
                                ))}
                                {interviewAny.consolidated_topics.length > 5 && (
                                  <Badge variant="outline">+{interviewAny.consolidated_topics.length - 5}</Badge>
                                )}
                              </div>
                            </div>
                          )}
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            {interviewAny.average_urgency && (
                              <div>
                                <div className="text-xs text-muted-foreground mb-1">Urgencia Promedio</div>
                                <div className="flex items-center gap-2">
                                    ...
                                    {Array.from({ length: 5 }).map((_, i) => (
                                      <div
                                        ...
                                        className={`h-2.5 w-2.5 rounded-full ${
                                          i < Math.round(interviewAny.average_urgency)
                                            ? 'bg-amber-400'
                                            : 'bg-muted'
                                        }`}
                                      />
                                  ...
                                  <span className="text-sm font-medium">{interviewAny.average_urgency}/5</span>
                                </div>
                              </div>
                            )}
                            
                            {interviewAny.average_productivity_impact && (
                              <div>
                                <div className="text-xs text-muted-foreground mb-1">Impacto Productividad</div>
                                <div className="flex items-center gap-2">
                                  <div className="flex gap-0.5">
                                    {Array.from({ length: 5 }).map((_, i) => (
                                      <div
                                        key={i}
                                        className={`h-2.5 w-2.5 rounded-full ${
                                          i < Math.round(interviewAny.average_productivity_impact)
                                            ? 'bg-blue-400'
                                            : 'bg-muted'
                                        }`}
                                      />
                                  ...
                                  <span className="text-sm font-medium">{interviewAny.average_productivity_impact}/5</span>
                                </div>
                              </div>
                            )}
                          </div>
                        ...
                        {interviewAny.critical_issues && interviewAny.critical_issues.length > 0 && (
                          <div>
                            <h4 className="text-sm font-semibold mb-2 text-red-400">🚨 Problemas Críticos</h4>
                            <ul className="space-y-1">
                              {interviewAny.critical_issues.slice(0, 3).map((issue: string, i: number) => (
                                <li key={i} className="text-sm text-muted-foreground">
                                  • {issue}
                                </li>
                              ))}
                              {interviewAny.critical_issues.length > 3 && (
                                <li className="text-sm text-muted-foreground">
                                  ... y {interviewAny.critical_issues.length - 3} más
                                </li>
                              )}
                            </ul>
                          </div>
                        ...
                        {interviewAny.general_recommendations && interviewAny.general_recommendations.length > 0 && (
                          <div>
                            <h4 className="text-sm font-semibold mb-2 text-emerald-400">💡 Recomendaciones Principales</h4>
                            <ul className="space-y-1">
                              {interviewAny.general_recommendations.slice(0, 3).map((rec: string, i: number) => (
                                <li key={i} className="text-sm text-muted-foreground">
                                  • {rec}
                                </li>
                              ))}
                              {interviewAny.general_recommendations.length > 3 && (
                                <li className="text-sm text-muted-foreground">
                                  ... y {interviewAny.general_recommendations.length - 3} más
                                </li>
                              )}
                            </ul>
                          </div>
